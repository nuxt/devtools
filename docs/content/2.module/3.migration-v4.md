...
host. It still works as a shim, but emits the `NDT_DEP_0004` deprecation
diagnostic.
...
- )
+ const session = await nuxt.devtools.terminals.startChildProcess(
...
registration. It still works as a shim, but emits the `NDT_DEP_0003` deprecation
diagnostic.

Register functions on the connect-safe `nuxt.devtools.rpc.register()` instead —
it buffers registrations until the Vite DevTools kit connects, so it is safe to
call during module setup:
...
## `nuxt.devtools.rpc` is now the Vite DevTools RPC host {#ndt_dep_0007}

`nuxt.devtools.rpc` is now the devframe `RpcFunctionsHost` — it exposes
`register`, `update`, `has`, `get`, `list`, `invokeLocal`, `broadcast`,
`sharedState`, `streaming`, and more, forwarding to the Vite DevTools kit (calls
made before it connects are buffered and replayed).

The legacy compatibility API still works but is **deprecated** (`NDT_DEP_0007`):

- **Broadcast** — the `rpc.broadcast.<method>.asEvent(...)` proxy form is
  deprecated; call `broadcast` natively instead:

  ```diff
  - nuxt.devtools.rpc.broadcast.myEvent.asEvent(payload)
  + nuxt.devtools.rpc.broadcast({ method: 'myEvent', args: [payload], event: true })
  ```

- **Functions** — the `rpc.functions` read/write proxy is deprecated; register
  and invoke functions via the host instead:

  ```diff
  - nuxt.devtools.rpc.functions.myFn = handler
  + nuxt.devtools.rpc.register({ name: 'myFn', handler })

  - const result = await nuxt.devtools.rpc.functions.myFn()
  + const result = await nuxt.devtools.rpc.invokeLocal('myFn')
  ```

## `addCustomTab()` is deprecated {#ndt_dep_0005}

`addCustomTab()` is soft-deprecated in favour of registering a dock entry on the
Vite DevTools docks host. It still works as a shim, but emits the `NDT_DEP_0005`
...
```ts
const entry = nuxt.devtools.docks.register({ id: 'my-module', /* ... */ })

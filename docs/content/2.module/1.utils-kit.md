...
### `NUXT_DEVTOOLS_GROUP_ID`

The public `Nuxt` dock group id registered on the Vite DevTools framework
category. `nuxt:devtools` is its default member. Module authors join the
group natively by pointing their own dock entries at it — no special Nuxt API
required:

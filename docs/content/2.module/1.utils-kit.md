...
`ViteDevToolsNodeContext`, so the full devframe surface is guaranteed available —
no connect-safe buffering needed. Do all your DevTools integration here:
...
### Devframe host accessors (connect-safe)

If you need to touch the DevTools surface _outside_ `onDevtoolsReady()` (e.g.
directly in module `setup`, before the kit connects), the same devframe-native
surface is also exposed **connect-safe** on `nuxt.devtools`:
...
- `nuxt.devtools.rpc` — the devframe RPC host (`register`/`invokeLocal`/`broadcast`/`sharedState`/`streaming`/…); the forward path for the deprecated `extendServerRpc`

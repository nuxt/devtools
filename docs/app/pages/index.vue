...
<template>
  ...
  <UPageHero orientation="horizontal">
    ...
    <template #links>
      ...
      <UButton size="xl" color="neutral" variant="outline" icon="i-ph-video-duotone" @click="videoModalOpen = true">
        What is Nuxt DevTools?
      </UButton>
      <UModal v-model:open="videoModalOpen">
        <template #content>
          <div class="p-4">
            <iframe
              width="100%"
  ...
  <UPageSection v-for="(section, index) of page.sections" :key="index" v-bind="section">
    ...
    <!-- tools -->
    <UPageGrid v-if="section.slot === 'tools'">
      <UPageCard
        v-for="card in section.toolsCards" :key="card.title" :to="card.to" :icon="card.icon"
        :title="card.title" :description="card.description"
        class="from-gray-900 to-gray-950 bg-gradient-to-b hover:bg-gray-100/50 hover:ring-2 hover:ring-gray-500"
      />
    </UPageGrid>

    <!-- project -->
    <div v-else-if="section.slot === 'project'">
      <div ref="nuxtProjectsSection" class="flex flex-row gap-x-12">
        <ul class="flex flex-col items-center justify-center lg:w-[40%]">
          <li v-for="(project, idx) in section.projectCards" :key="idx">
            <div
              class="group relative hidden cursor-pointer rounded-lg p-4 lg:block"
              @click="selectProjectCard(idx)"
            >
              <h4
              ...
              <p
                class="mt-2 transition-color duration-200 group-hover:text-gray-400"
                :class="currentStep === idx ? 'text-gray-400' : 'text-gray-600'"
              >
                {{ project.description }}
              </p>
              <UButton
                trailing variant="link" color="neutral" size="md"
                class="z-20 mt-2 -ml-2.5" :to="project.to"
              >
      ...
      <div class="w-full flex justify-center pt-8">
        <UButton size="xl" variant="outline" color="neutral" to="/guide/features">
          {{ section.button }}
        </UButton>
      </div>
    </div>
    ...
    <!-- cta -->
    <UPageCTA
      v-else-if="section.slot === 'cta'"
      align="left" card
      class="from-gray-900 to-gray-950 bg-gradient-to-b"
    >
      <template #title>
        <span v-html="section.title" />
      </template>
    ...
    <!-- get-started -->
    <div v-else-if="section.slot === 'get-started'" class="w-full flex flex-col items-center justify-center">
      ...
      <UButton to="/guide/getting-started" size="xl" :label="section.button" variant="outline" color="neutral" class="mt-8 w-fit" />
    </div>
  </UPageSection>
</template>
